...
import IUser from "../../data/user/user.interface";
...
export const Authenticate = async (uid: string, name: string, fire_token: string, callback: (error: string | null, user: IUser | null) => void) => {
    try {
        ...
        if (response.status === 200 || response.status === 201 || response.status === 304) {
            console.info('Successfully authenticated.', NAMESPACE);
            callback(null, response.data.user);
        } else {
            console.warn('Unable to authenticate.', NAMESPACE);
            callback('Unable to authenticate.', null);
        }
    } catch (error) {
        console.error(error, NAMESPACE);
        callback('Unable to authenticate.', null);
    }
};
...
export const Validate = async (fire_token: string, callback: (error: string | null, user: IUser | null) => void) => {
    try {
        ...
        if (response.status === 200 || response.status === 304) {
            console.info('Successfully validated.', NAMESPACE);
            callback(null, response.data.user);
        } else {
            console.warn(response, NAMESPACE);
